...
class PostgresSkillRepository:
    ...
    async def get_by_key(self,key): result=await self.session.execute(select(SkillRecord).where(SkillRecord.key==key)); r=result.scalar_one_or_none(); return _skill_to_domain(r) if r else None
    async def create(self,s): self.session.add(SkillRecord(id=s.id,key=s.key,name=s.name,description=s.description,objective=s.objective,instructions=s.instructions,inputs=s.inputs,output_schema=s.output_schema,knowledge_sources=s.knowledge_sources,allowed_tools=s.allowed_tools,constraints=s.constraints,version=s.version,enabled=s.enabled)); await self.session.commit(); return s
    async def update(self,s): result=await self.session.execute(select(SkillRecord).where(SkillRecord.key==s.key)); r=result.scalar_one(); r.name=s.name; r.description=s.description; r.objective=s.objective; r.instructions=s.instructions; r.inputs=s.inputs; r.output_schema=s.output_schema; r.knowledge_sources=s.knowledge_sources; r.allowed_tools=s.allowed_tools; r.constraints=s.constraints; r.version=s.version; r.enabled=s.enabled; await self.session.commit(); return s
class PostgresExecutionRepository:
    def __init__(self,session:AsyncSession): self.session=session
    async def get(self,execution_id): r=await self.session.get(ExecutionRecord,execution_id); return _execution_to_domain(r) if r else None
    async def create(self,e): self.session.add(ExecutionRecord(id=e.id,correlation_id=e.correlation_id,agent_key=e.agent_key,skill_key=e.skill_key,status=e.status.value,objective=e.objective,runtime=e.runtime,model=e.model,started_at=e.started_at,completed_at=e.completed_at,created_at=e.created_at,usage=e.usage.model_dump(mode="json"),provider_request_id=e.provider_request_id,trace_id=e.trace_id,error=e.error.model_dump(mode="json") if e.error else None)); await self.session.commit(); return e
    async def update(self,e): r=await self.session.get(ExecutionRecord,e.id); r.status=e.status.value; r.runtime=e.runtime; r.model=e.model; r.started_at=e.started_at; r.completed_at=e.completed_at; r.usage=e.usage.model_dump(mode="json"); r.provider_request_id=e.provider_request_id; r.trace_id=e.trace_id; r.error=e.error.model_dump(mode="json") if e.error else None; await self.session.commit(); return e
    async def add_event(self,execution_id,event_type,payload): self.session.add(ExecutionEventRecord(execution_id=execution_id,event_type=event_type,payload=payload,created_at=datetime.now(timezone.utc))); await self.session.commit()
    async def list_events(self,execution_id): result=await self.session.execute(select(ExecutionEventRecord).where(ExecutionEventRecord.execution_id==execution_id).order_by(ExecutionEventRecord.created_at)); return [{"event_type":r.event_type,"payload":r.payload,"created_at":r.created_at.isoformat()} for r in result.scalars().all()]
class PostgresKnowledgeRepository:
    ...
    async def list_chunks(self,document_id): result=await self.session.execute(select(KnowledgeChunkRecord).where(KnowledgeChunkRecord.document_id==document_id).order_by(KnowledgeChunkRecord.ordinal)); return [_chunk_to_domain(r) for r in result.scalars().all()]
