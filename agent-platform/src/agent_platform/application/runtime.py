...
class AgentRuntimeValidationError(RuntimeError):
    pass
...
class AgentRuntime:
    ...
    async def execute(self, request: AgentExecutionRequest) -> AgentExecutionResult:
        ...
        if not agent.enabled:
            raise AgentRuntimeValidationError("Agent is disabled")

        skill = None
        if request.skill_key is not None:
            skill = await self._skills.get(request.skill_key)
            if not skill.enabled:
                raise AgentRuntimeValidationError("Skill is disabled")
            if request.skill_key not in agent.skills:
                raise AgentRuntimeValidationError("Skill is not assigned to agent")
        ...
        await self._record(execution, "execution.queued")

        running = execution.model_copy(
            update={
                "status": ExecutionStatus.RUNNING,
                "runtime": "native-python-v1",
        ...
        try:
            ...
            return await self._fail(running, "AGENT_RUNTIME_ERROR", str(exc) or type(exc).__name__)
    ...
    async def _fail(
        ...
        *,
        ...
        await self._executions.update(failed)
        await self._record(failed, "execution.failed")
        return AgentExecutionResult(
            execution_id=failed.id,
            status=failed.status,
            usage=failed.usage,
            model=failed.model,
            provider_request_id=failed.provider_request_id,
            trace_id=failed.trace_id,
            error=error,
        )

    async def _record(self, execution: AgentExecution, event_type: str) -> None:
        await self._executions.add_event(execution.id, event_type, execution.model_dump(mode="json"))
