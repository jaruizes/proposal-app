...
class AgentRuntime:
    def __init__(self,agents:AgentRegistry,skills:SkillRegistry,executions:ExecutionRepository,model_provider:ModelProvider,prompt_assembler:AgentPromptAssembler|None=None,cognitive_context_builder:CognitiveContextBuilder|None=None,memory_service:MemoryService|None=None,cache:CacheService|None=None)->None:
        ...
        try:
            ...
            if self._cache is not None and cache_config["enabled"]:
                ...
                if cached is not None:
                    ...
                    model_result=cached_result.model_copy(update={"usage":ModelUsage(),"provider_request_id":None,"metadata":{**cached_result.metadata,"platform_cache_hit":True}})
                    cache_hit=True
                    await self._executions.add_event(running.id,"execution.cache.hit",{"key":cache_key})
                else:
                    await self._executions.add_event(running.id,"execution.cache.miss",{"key":cache_key})
            ...
            if self._memory_service is not None and self._should_auto_capture(skill):
                try: await self._memory_service.capture_execution(correlation_id=request.correlation_id,agent_key=agent.key,skill_key=skill.key if skill else None,execution_id=completed.id,content=model_result.content,model=model_result.model)
    ...
    @staticmethod
    def _execution_cache_config(skill)->dict:
        if skill is None or not isinstance(skill.constraints,dict): return {"enabled":False,"ttl_seconds":900}
        cache=skill.constraints.get("cache",{}); execution=cache.get("execution",{}) if isinstance(cache,dict) else {}
        return {"enabled":bool(execution.get("enabled",False)),"ttl_seconds":max(1,int(execution.get("ttl_seconds",900)))}
