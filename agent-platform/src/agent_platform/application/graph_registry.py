...
class GraphRegistry:
    ...
    def attach(cls, graph_key: str, builder, runtime, execution) -> None:
        ...
        if graph_key == "presentation-materialization":
            ...
            return
        if graph_key == "source-ingestion":
            from agent_platform.application.source_ingestion_graph import add_source_ingestion_nodes
            add_source_ingestion_nodes(builder, runtime, execution)
            return
        raise ValueError(
