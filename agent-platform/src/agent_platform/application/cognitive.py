...
class MemoryContextContributor:
    ...
    async def contribute(self, agent, skill, request):
        ...
        if request.correlation_id is not None:
            scopes.append(MemoryScope(type=MemoryScopeType.CORRELATION, key=str(request.correlation_id)))
        if bool(config.get("include_agent_scope", False)):
            scopes.append(MemoryScope(type=MemoryScopeType.AGENT, key=agent.key))
        if bool(config.get("include_global", False)):
            scopes.append(MemoryScope(type=MemoryScopeType.GLOBAL, key="*"))
        for custom in config.get("custom_scopes", []) or []:
            if isinstance(custom, str) and custom.strip():
                scopes.append(MemoryScope(type=MemoryScopeType.CUSTOM, key=custom.strip()))
        if not scopes:
            return []
        ...
        for rank, entry in enumerate(entries, start=1):
            items.append(CognitiveContextItem(key=f"memory:{entry.id}", section=CognitiveSection.MEMORY, content={"kind": entry.kind.value, "content": entry.content}, label=label_by_kind.get(entry.kind, EpistemicLabel.UNCLASSIFIED), source=f"memory://{entry.id}", priority=max(70, 86-rank), required=False, metadata={"memory_id": str(entry.id), "scope_type": entry.scope_type.value, "scope_key": entry.scope_key, "importance": entry.importance, **entry.metadata}))
        return items
