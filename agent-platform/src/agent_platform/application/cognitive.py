...
class ApplicationContextContributor:
    name="application-context";required=True
...
class MemoryContextContributor:
    ...
    def __init__(self,service:MemoryService)->None:self._service=service
    async def contribute(self,agent,skill,request):
        ...
        entries=await self._service.recall(MemoryRecall(scopes=scopes,kinds=kinds,agent_key=agent.key if bool(config.get("same_agent_only",False)) else None,skill_key=skill.key if skill and bool(config.get("same_skill_only",False)) else None,min_importance=float(config.get("min_importance",0.0)),limit=int(config.get("max_items",6))))
        labels={MemoryKind.FACT:EpistemicLabel.FACT,MemoryKind.DECISION:EpistemicLabel.DECISION}
        return[CognitiveContextItem(key=f"memory:{entry.id}",section=CognitiveSection.MEMORY,content={"kind":entry.kind.value,"content":entry.content},label=labels.get(entry.kind,EpistemicLabel.UNCLASSIFIED),source=f"memory://{entry.id}",priority=max(70,86-rank),required=False,metadata={"memory_id":str(entry.id),"scope_type":entry.scope_type.value,"scope_key":entry.scope_key,"importance":entry.importance,**entry.metadata}) for rank,entry in enumerate(entries,start=1)]


class KnowledgeRetrievalContributor:
    name="knowledge-retrieval";required=False
    def __init__(self,service:KnowledgeRetrievalService,cache:CacheService|None=None,*,cache_ttl_seconds:int=900)->None:self._service=service;self._cache=cache;self._cache_ttl_seconds=cache_ttl_seconds
    ...
    async def contribute(self,agent,skill,request):
        ...
        cache_key=stable_cache_key({"agent":[agent.key,agent.version],"skill":[skill.key,skill.version],"objective":request.objective,"knowledge_sources":skill.knowledge_sources,"retrieval":config})
        if self._cache is not None:
            cached=await self._cache.get_json("cognitive-rag",cache_key)
            if cached is not None:return[CognitiveContextItem.model_validate(item).model_copy(update={"metadata":{**item.get("metadata",{}),"cognitive_cache":"hit"}}) for item in cached]
        mode=RetrievalMode(str(config.get("mode",RetrievalMode.HYBRID.value)));top_k=int(config.get("top_k",5));candidate_k=int(config.get("candidate_k",max(20,top_k)))
        result=await self._service.retrieve(RetrievalQuery(
            text=self._query_text(skill,request),mode=mode,top_k=top_k,candidate_k=candidate_k,
            ...
            vector_weight=float(config.get("vector_weight",1.0)),keyword_weight=float(config.get("keyword_weight",1.0)),
            ontology_enabled=bool(config.get("ontology_enabled",True)),ontology_weight=float(config.get("ontology_weight",1.0)),
            ontology_max_hops=int(config.get("ontology_max_hops",1)),ontology_max_concepts=int(config.get("ontology_max_concepts",12)),
        ))
        items=[]
        for rank,hit in enumerate(result.hits,start=1):
            source=hit.source_uri or f"knowledge://{hit.knowledge_base_key}/{hit.document_id}"
            items.append(CognitiveContextItem(key=f"knowledge:{hit.chunk_id}",section=CognitiveSection.RETRIEVED_KNOWLEDGE,content={"title":hit.title,"excerpt":hit.content,"parent_context":hit.parent_content,"source_uri":hit.source_uri},label=EpistemicLabel.REFERENCE,source=source,priority=max(55,76-rank),required=False,metadata={"rank":rank,"score":hit.score,"retrieval_method":hit.retrieval_method.value,"retrieval_query":result.query,"knowledge_base_key":hit.knowledge_base_key,"document_id":str(hit.document_id),"chunk_id":str(hit.chunk_id),"chunk_metadata":hit.metadata,"parent_chunk_id":str(hit.parent_chunk_id) if hit.parent_chunk_id else None,"parent_metadata":hit.parent_metadata,"embedding_model":result.embedding_model,"ontology_seed_concepts":result.metadata.get("ontology_seed_concepts",[]),"ontology_expanded_concepts":result.metadata.get("ontology_expanded_concepts",[]),"cognitive_cache":"miss" if self._cache is not None else "disabled"}))
        if self._cache is not None:await self._cache.set_json("cognitive-rag",cache_key,[item.model_dump(mode="json") for item in items],ttl_seconds=self._cache_ttl_seconds)
        return items

    @staticmethod
    def _query_text(skill,request):
        task=request.objective.strip();objective=skill.objective.strip();return task if not objective or objective.lower()==task.lower() else f"{objective}\n\nCurrent task: {task}"


class CognitiveContextPolicy:
    def __init__(self,*,max_items:int=50,max_context_chars:int=80_000)->None:self.max_items=max_items;self.max_context_chars=max_context_chars


class CognitiveContextBuilder:
    def __init__(self,contributors:list[ContextContributor]|None=None,policy:CognitiveContextPolicy|None=None)->None:self._contributors=contributors if contributors is not None else[ApplicationContextContributor(),AttachmentContextContributor()];self._policy=policy or CognitiveContextPolicy()
    async def build(self,agent,skill,request):
        ...
        results=await asyncio.gather(*(run(c) for c in self._contributors));groups=[items for items,_ in results];warnings=[w for _,w in results if w];candidates=[item for group in groups for item in group];selected,dropped=self._apply_budget(candidates)
