...
class KnowledgeService:
    ...
    async def bootstrap_defaults(self):
        ...
        for key, name, description in DEFAULT_KNOWLEDGE_BASES:
            if await self._repository.get_base(key) is None:
                created.append(await self._repository.create_base(KnowledgeBase(key=key, name=name, description=description)))
        return created
    ...
    async def upload_document(self, knowledge_base_key: str, document: KnowledgeDocument) -> KnowledgeDocument:
        ...
        return await self._repository.create_document(document.model_copy(update={"knowledge_base_key": knowledge_base_key}))
