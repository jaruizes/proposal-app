...
class KnowledgeIngestionService:
    def __init__(self,repository:KnowledgeRepository,embedding_provider:EmbeddingProvider,*,normalizer:TextNormalizer|None=None,metadata_enricher:MetadataEnricher|None=None,cache:CacheService|None=None)->None:
        self._repository=repository; self._embedding_provider=embedding_provider; self._normalizer=normalizer or TextNormalizer(); self._metadata_enricher=metadata_enricher or DeterministicMetadataEnricher(); self._cache=cache

    async def _invalidate_knowledge_cache(self)->None:
        if self._cache is None: return
        await self._cache.invalidate_namespace("retrieval")
        await self._cache.invalidate_namespace("cognitive-rag")
    ...
    async def ingest(self,document_id:UUID,*,chunking_strategy:ChunkingStrategyName|str=ChunkingStrategyName.FIXED,chunk_size:int=1200,overlap:int=200,parent_size:int=6000,child_size:int=1200,child_overlap:int=200,embed:bool=True,metadata_enrichment:MetadataEnrichmentProfile|str=MetadataEnrichmentProfile.STANDARD,max_keywords:int=8)->KnowledgeIngestionResult:
        ...
        try:
            ...
            enriched_document=document.model_copy(update={"metadata":self._merge_enrichment(document.metadata,document_enrichment),"status":KnowledgeDocumentStatus.PROCESSING})
            ...
            await self._repository.update_document(enriched_document.model_copy(update={"status":KnowledgeDocumentStatus.READY}))
    ...
    async def enrich_existing(self,document_id:UUID,*,metadata_enrichment:MetadataEnrichmentProfile|str=MetadataEnrichmentProfile.STANDARD,max_keywords:int=8)->KnowledgeMetadataEnrichmentResult:
        ...
        for chunk in chunks:
            metadata=await self._metadata_enricher.enrich_chunk(updated,chunk,profile=selected,max_keywords=max_keywords,document_enrichment=doc_enrichment)
            enriched.append(chunk.model_copy(update={"metadata":self._merge_enrichment(chunk.metadata,metadata)}))
