...
class AnthropicModelProvider:
    """Anthropic adapter with bounded platform-owned retry semantics."""
    ...
    def __init__(self,settings:Settings|None=None,client:Any|None=None)->None:
        ...
        if self._settings.anthropic_base_url:kwargs["base_url"]=self._settings.anthropic_base_url
    ...
    async def generate(self,request:ModelRequest)->ModelResult:
        ...
        attempts=max(1,self._settings.anthropic_max_retries+1)
        last_exc=None
        for attempt in range(attempts):
            try:
                ...
                if not retryable or attempt==attempts-1:
                    raise ModelProviderError(f"ANTHROPIC_{name.upper()}",str(exc) or "Anthropic request failed",retryable=retryable) from exc
                await asyncio.sleep(self._settings.anthropic_retry_base_seconds*(2**attempt))
        else:
            raise ModelProviderError("ANTHROPIC_REQUEST_FAILED",str(last_exc) if last_exc else "Anthropic request failed",retryable=True)
