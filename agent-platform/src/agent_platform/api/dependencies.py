...
def get_embedding_provider() -> EmbeddingProvider:
    settings=get_settings(); return HashEmbeddingProvider(dimensions=settings.embedding_dimensions,model=settings.embedding_model)
EmbeddingProviderDep=Annotated[EmbeddingProvider,Depends(get_embedding_provider)]

async def get_knowledge_ingestion_service(repository: KnowledgeRepositoryDep, embedding_provider: EmbeddingProviderDep) -> KnowledgeIngestionService: return KnowledgeIngestionService(repository,embedding_provider)
...
async def get_knowledge_retrieval_service(session: Annotated[AsyncSession,Depends(get_session)],embedding_provider: EmbeddingProviderDep) -> KnowledgeRetrievalService: return KnowledgeRetrievalService(PostgresKnowledgeSearchBackend(session),embedding_provider)
KnowledgeRetrievalServiceDep=Annotated[KnowledgeRetrievalService,Depends(get_knowledge_retrieval_service)]

async def get_cognitive_context_builder(retrieval_service: KnowledgeRetrievalServiceDep,memory_service: MemoryServiceDep) -> CognitiveContextBuilder:
    return CognitiveContextBuilder(contributors=[ApplicationContextContributor(),AttachmentContextContributor(),MemoryContextContributor(memory_service),KnowledgeRetrievalContributor(retrieval_service)])
CognitiveContextBuilderDep=Annotated[CognitiveContextBuilder,Depends(get_cognitive_context_builder)]

def get_agent_runtime(agents: AgentRegistryDep,skills: SkillRegistryDep,executions: ExecutionRepositoryDep,model_provider: ModelProviderDep,cognitive_context_builder: CognitiveContextBuilderDep,memory_service: MemoryServiceDep) -> AgentRuntime:
    return AgentRuntime(agents,skills,executions,model_provider,cognitive_context_builder=cognitive_context_builder,memory_service=memory_service)
