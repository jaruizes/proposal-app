...
def get_cache_service()->CacheService:
    ...
    provider=ValkeyCacheProvider(settings.cache_url) if settings.cache_backend.lower() in {"valkey","redis"} else InMemoryCacheProvider()
    return CacheService(provider,prefix=settings.cache_prefix)
...
def get_embedding_provider()->EmbeddingProvider:
    settings=get_settings(); base=HashEmbeddingProvider(dimensions=settings.embedding_dimensions,model=settings.embedding_model)
    return CachedEmbeddingProvider(base,get_cache_service(),ttl_seconds=settings.embedding_cache_ttl_seconds)
EmbeddingProviderDep=Annotated[EmbeddingProvider,Depends(get_embedding_provider)]

async def get_knowledge_ingestion_service(repository:KnowledgeRepositoryDep,embedding_provider:EmbeddingProviderDep,cache:CacheServiceDep)->KnowledgeIngestionService:return KnowledgeIngestionService(repository,embedding_provider,cache=cache)
...
async def get_cognitive_context_builder(retrieval_service:KnowledgeRetrievalServiceDep,memory_service:MemoryServiceDep,cache:CacheServiceDep)->CognitiveContextBuilder:
    settings=get_settings()
    return CognitiveContextBuilder(contributors=[ApplicationContextContributor(),AttachmentContextContributor(),MemoryContextContributor(memory_service),KnowledgeRetrievalContributor(retrieval_service,cache,cache_ttl_seconds=settings.cognitive_cache_ttl_seconds)])
CognitiveContextBuilderDep=Annotated[CognitiveContextBuilder,Depends(get_cognitive_context_builder)]

def get_agent_runtime(agents:AgentRegistryDep,skills:SkillRegistryDep,executions:ExecutionRepositoryDep,model_provider:ModelProviderDep,cognitive_context_builder:CognitiveContextBuilderDep,memory_service:MemoryServiceDep,cache:CacheServiceDep)->AgentRuntime:
    return AgentRuntime(agents,skills,executions,model_provider,cognitive_context_builder=cognitive_context_builder,memory_service=memory_service,cache=cache)
