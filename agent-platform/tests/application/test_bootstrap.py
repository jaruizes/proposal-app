...
def test_bootstrap_catalog_replicates_current_proposal_copilot_definitions() -> None:
    ...
    assert "ingest-sources" in business_analyst.skills
    ...
    presentation = next(skill for skill in skills if skill.key == "generate-presentation")
    assert presentation.constraints["execution"]["graph"] == "presentation-materialization"
    ingestion = next(skill for skill in skills if skill.key == "ingest-sources")
    assert ingestion.constraints["execution"]["graph"] == "source-ingestion"
