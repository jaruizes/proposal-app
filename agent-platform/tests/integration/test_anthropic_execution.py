...
from agent_platform.api.dependencies import get_agent_repository, get_execution_repository, get_model_provider, get_skill_repository
...
class InMemoryAgentRepository:
    def __init__(self, items: dict[str, AgentDefinition]) -> None: self.items = items
...
class InMemorySkillRepository:
    def __init__(self, items: dict[str, SkillDefinition]) -> None: self.items = items
    async def list(self): return list(self.items.values())
    async def get_by_key(self, key): return self.items.get(key)
    async def create(self, item): self.items[item.key] = item; return item
    async def update(self, item): self.items[item.key] = item; return item
...
def test_post_execution_with_bootstrapped_business_analyst_and_real_anthropic() -> None:
    ...
    for agent in agents:
        if agent.key != "business-analyst": continue
        policy = agent.model_policy.model_copy(update={"preferred_model": model_override or agent.model_policy.preferred_model, "max_output_tokens": 256})
    ...
    settings = Settings(anthropic_api_key=api_key, anthropic_base_url=os.getenv("ANTHROPIC_BASE_URL") or None, anthropic_default_model=model_override or "claude-sonnet-4-6")
    ...
    try:
        with TestClient(app) as client:
            response = client.post("/v1/executions", json={
                ...
                "context": {"customer": "Parity Test Corp", "source_manifest": {"documents": []}, "note": "Integration smoke test; no real customer facts are supplied."},
        ...
        assert body["artifacts"] and body["artifacts"][0]["type"] == "AGENT_OUTPUT"
