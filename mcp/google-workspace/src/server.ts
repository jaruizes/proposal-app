...
server.tool("slides_duplicate_slide","Duplicate a slide within a generated presentation.",{presentationId:z.string(),slideObjectId:z.string()},async({presentationId,slideObjectId})=>text((await slides.presentations.batchUpdate({presentationId,requestBody:{requests:[{duplicateObject:{objectId:slideObjectId}}]}})).data.replies??[]));
