...
public final class OfferRestMapper {
    ...
    public static OfferResponse toResponse(Offer o,List<PhaseExecution> phases,List<Artifact> artifacts){
        ...
        for(var a:artifacts){byPhase.computeIfAbsent(a.phase(),ignored->new StringBuilder()).append("\n\n# ").append(a.type()).append(" · v").append(a.version()).append("\n\n").append(a.content());}
        var prs=phases.stream().sorted(Comparator.comparingInt(p->p.phase().ordinal())).map(p->new OfferResponse.PhaseResponse(key(p.phase()),p.phase().label(),phaseStatus(p.status()),Optional.ofNullable(byPhase.get(p.phase())).map(StringBuilder::toString).orElse(null))).toList();
        return new OfferResponse(o.id(),o.name(),o.customer(),o.presentationLanguage(),o.inputDriveFolder(),o.outputDriveFolder(),o.presentationName(),o.aiProvider(),o.models(),o.presentationGuidance(),o.currentPhase().label(),overall(o.status()),o.createdAt(),prs);
    }
    ...
    private static String phaseStatus(ExecutionStatus s){return switch(s){case RUNNING->"working";case WAITING_FOR_HUMAN->"waiting_approval";case APPROVED->"approved";case CANCELLED->"cancelled";default->"pending";};}
