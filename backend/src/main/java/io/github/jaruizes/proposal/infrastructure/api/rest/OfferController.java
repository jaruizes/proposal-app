...
@PostMapping public OfferResponse create(@Valid @RequestBody CreateOfferRequest r){var offer=workflow.create(new OfferWorkflowService.CreateOfferCommand(r.name(),blank(r.customer(),r.name()),blank(r.language(),"es"),blank(r.presentationLanguage(),"es"),blank(r.inputDriveFolder(),""),blank(r.outputDriveFolder(),""),blank(r.presentationName(),r.name()+" - Presentación"),r.generatePresentation()==null||r.generatePresentation(),blank(r.aiProvider(),"ANTHROPIC"),r.models()==null?Map.of():r.models(),r.proposalGuidance(),r.presentationGuidance()));return OfferRestMapper.toResponse(offer,workflow.phases(offer.id()),workflow.artifacts(offer.id()));}
...
public record UpdateOfferConfigurationRequest(String presentationLanguage,String inputDriveFolder,String outputDriveFolder,String presentationName,Boolean generatePresentation,String aiProvider,Map<String,String> models,Object proposalGuidance,Object presentationGuidance) {}
private static String blank(String v,String d){return v==null||v.isBlank()?d:v;} private static PhaseType parse(String p){return switch(p){case"analysis"->PhaseType.ANALYSIS;case"solution"->PhaseType.SOLUTION;case"proposal"->PhaseType.PROPOSAL;case"slide-plan"->PhaseType.SLIDE_PLAN;case"presentation"->PhaseType.PRESENTATION;default->throw new IllegalArgumentException("Unknown phase");};}}
